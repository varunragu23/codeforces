...
bool placeable(VVI& dom, const int& i, const int& j) {
  if (dom[i][j]) return true;
  ++dom[i][j];
  /*for (int k = 0; k < dom.size(); k++) {
    for (int l = 0; l < dom[0].size(); l++) {
      if (k ==i && j == l) {
        cout << "H" << " ";
      }
      else cout << dom[k][l] << " ";
  ...
  cout << "this was test case: " << i << " , " << j;
  cout << endl << endl;
  */
  if (i + 2 >= dom.size()) {
    return false;
  }
  if (j <= 0 || j>= (dom[0].size() - 1)) {
    return false;
  }
  bool ret = dom[i + 1][j] || dom[i + 2][j] || dom[i + 1][j + 1] || dom[i + 1][j - 1];
  ++dom[i + 1][j], ++dom[i + 2][j], ++dom[i + 1][j + 1], ++dom[i + 1][j - 1];
  return !ret;
}
